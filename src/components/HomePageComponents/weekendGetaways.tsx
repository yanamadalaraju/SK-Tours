...
const WeekendGetawaysSection: React.FC = () => {
  const [weekendGetaways, setWeekendGetaways] = useState<WeekendGetaway[]>([]);
  ...
  // Handle view package click
  const handleViewPackage = (gatewayId: number) => {
  ...
  useEffect(() => {
    ...
    if (isAutoPlaying && weekendGetaways.length > 1) {
      interval = setInterval(() => {
        setCurrentIndex((prevIndex) => 
          prevIndex === weekendGetaways.length - 1 ? 0 : prevIndex + 1
        );
      }, 3000);
    }
  ...
  }, [isAutoPlaying, weekendGetaways.length]);
  ...
  const prevSlide = () => {
    setIsAutoPlaying(false);
    setCurrentIndex((prevIndex) => 
      prevIndex === 0 ? weekendGetaways.length - 1 : prevIndex - 1
    );
    ...
    return (
      <section className="py-0 bg-gradient-to-br from-sky-200 via-sky-200 to-sky-200">
        <div className="w-full bg-gradient-to-r from-[#0F1F5C] via-[#1F3F93] to-[#0F1F5C] py-8 mb-10 shadow-lg">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center">
              <h2 className="text-4xl md:text-6xl font-extrabold mb-4 tracking-tight" style={{ fontFamily: "'Baloo 2', sans-serif", color: "white", textShadow: "2px 2px 0 #1F3F93, -2px -2px 0 #1F3F93, 0 0 18px rgba(255,255,255,0.65)" }}>
                Weekend <span className="text-[#E31B23]">Getaways</span>
              </h2>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl shadow-lg p-12 mb-10 bg-gradient-to-r from-[#0F1F5C] to-[#1F3F93] text-center">
            <div className="text-white">Loading amazing getaways...</div>
          </div>
        </div>
      </section>
    );
  ...
  if (weekendGetaways.length === 0) {
    return (
      <section className="py-0 bg-gradient-to-br from-sky-200 via-sky-200 to-sky-200">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl shadow-lg p-12 mb-10 bg-gradient-to-r from-[#0F1F5C] to-[#1F3F93] text-center">
            <div className="text-white">No getaways available at the moment</div>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section className="py-0 bg-gradient-to-br from-sky-200 via-sky-200 to-sky-200">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Carousel Container with same gradient as Domestic Tours */}
        <div
              ...
              <div className="flex justify-center">
                {weekendGetaways.map((getaway, index) => (
                  <div
                    ...
                    style={{ width: '280px' }} // Same width as Domestic Tours
                    onMouseEnter={handleCardMouseEnter}
                    onMouseLeave={handleCardMouseLeave}
                  >
                    <div className="group cursor-pointer bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 h-full border border-gray-100 flex flex-col">
                      {/* Getaway Image - same height as Domestic Tours */}
                      <div className="relative h-48 overflow-hidden flex-shrink-0">
                        <img
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = "https://images.pexels.com/photos/1365425/pexels-photo-1365425.jpeg?auto=compress&cs=tinysrgb&w=1200";
                          }}
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
                        
                        {/* Gateway Code Badge - similar to tour_id in Domestic Tours */}
                        {getaway.gateway_code && (
                          <div className="absolute top-2 left-2 z-10">
                      ...
                      {/* Content Section - matching Domestic Tours layout */}
                      <div className="p-4 flex flex-col flex-grow">
                        ...
                        <div className="flex items-center gap-1 text-gray-600 mb-2">
                          <MapPin className="h-3 w-3" />
                          <span className="text-xs">Weekend Gateway</span>
                        </div>
                        
                        <div className="mb-3 mt-auto">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-semibold text-gray-700">Package Cost</span>
                            <p className="text-lg md:text-2xl font-bold text-gray-900">{getaway.price}</p>
                          </div>
                        </div>
                        
                        <button
                          onClick={() => handleViewPackage(getaway.id)}
