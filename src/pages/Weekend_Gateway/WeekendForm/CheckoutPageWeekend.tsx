// src/pages/CheckoutPageWeekend.tsx
...
const CheckoutPageWeekend: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state?.weekend) {
      const weekend = location.state.weekend;
      setWeekendData(weekend);
      
      // Pre-fill form with weekend contact info
      setFormData(prev => ({
        ...prev,
        email: weekend.email_id || '',
        phone: weekend.cell_no || '',
        address: weekend.address || '',
        city: weekend.city || '',
        state: weekend.state || '',
        pincode: weekend.pin_code || '',
        country: weekend.country || 'India',
        firstName: weekend.contact_person?.split(' ')[0] || '',
        lastName: weekend.contact_person?.split(' ')[1] || '',
      }));
      
      const totalCost = weekend.total_price_value || 3500 * (weekend.no_of_rooms || 1);
      setCustomPaymentAmount(totalCost.toString());
      setPaymentType('full');
      setLoading(false);
    } else {
      const savedWeekend = localStorage.getItem('selectedWeekend');
      if (savedWeekend) {
        const parsedWeekend = JSON.parse(savedWeekend);
        setWeekendData(parsedWeekend);
        
        setFormData(prev => ({
          ...prev,
          email: parsedWeekend.email_id || '',
          phone: parsedWeekend.cell_no || '',
          address: parsedWeekend.address || '',
          city: parsedWeekend.city || '',
          state: parsedWeekend.state || '',
          pincode: parsedWeekend.pin_code || '',
          firstName: parsedWeekend.contact_person?.split(' ')[0] || '',
          lastName: parsedWeekend.contact_person?.split(' ')[1] || '',
        }));
        
        const totalCost = parsedWeekend.total_price_value || 3500 * (parsedWeekend.no_of_rooms || 1);
        setCustomPaymentAmount(totalCost.toString());
        setPaymentType('full');
      }
      setLoading(false);
  ...
  const handleCustomAmountChange = (value: string) => {
    ...
    if (weekendData) {
      const totalCost = getTotalWeekendCost();
  ...
  const getTotalWeekendCost = () => {
    if (!weekendData) return 0;
    return weekendData.total_price_value || 3500 * (weekendData.no_of_rooms || 1);
  };
  ...
  const validateCustomAmount = () => {
    ...
    if (enteredAmount > maxAmount) {
      alert(`Amount cannot exceed total weekend getaway cost of ${formatPrice(maxAmount)}`);
      return false;
    }
  ...
  const calculatePercentage = (amount: number) => {
    const total = getTotalWeekendCost();
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      const paymentDescription = isFullPayment 
        ? `Full Payment for Weekend Gateway - ${weekendData?.property_name}`
        : `${paymentPercentage}% Partial Payment for Weekend Gateway - ${weekendData?.property_name}`;
      
      // Prepare checkout payload with required backend fields
      const checkoutPayload = {
        // Backend required fields
        tour_id: weekendData?.id || weekendData?.code || 'weekend_001',
        total_tour_cost: totalCost,
        advance_amount: paymentAmount,
        
        // Weekend specific fields
        weekend_id: weekendData?.id || '',
        weekend_code: weekendData?.code || '',
        weekend_title: weekendData?.title || '',
        property_name: weekendData?.property_name || '',
        weekend_city: weekendData?.city || '',
        weekend_address: weekendData?.address || '',
        city_location: weekendData?.city_location || '',
        
        // Price details
        advance_percentage: paymentPercentage,
        no_of_rooms: weekendData?.no_of_rooms || 1,
        price_per_room: weekendData?.price_per_room || 3500,
        ...
        // Guest details
        no_of_adults: weekendData?.no_of_adults || 1,
        no_of_children: weekendData?.no_of_children || 0,
        total_people: weekendData?.total_people || 1,
        guests: JSON.stringify(weekendData?.guests || []),
        
        payment_method: formData.paymentMethod,
        source_page: 'weekend-gateway-booking',
        source: 'weekend',
      ...
      const merchantOrderId = `WEEKEND_${checkoutId}_${Date.now()}`;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-2xl shadow-lg text-center max-w-md">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">No Weekend Gateway Selected</h2>
          <p className="text-gray-600 mb-6">Please select a weekend gateway package to proceed with booking.</p>
          <Button 
            onClick={() => navigate('/weekend-gateway')} 
            className="bg-[#2E4D98] hover:bg-[#2E4D98]/90 px-8 py-6 text-lg"
          >
            Browse Weekend Gateways
          </Button>
        </div>
      </div>
    );
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h1 className="text-3xl font-bold text-gray-800 mb-2">Complete Your Weekend Gateway Booking</h1>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-5 mb-8 border border-purple-100">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
                  ...
                  Weekend Gateway Summary
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="font-bold text-lg text-gray-800 mb-2">{weekendData.property_name}</h3>
                    <p className="text-gray-600 mb-3">{weekendData.city}, {weekendData.state}</p>
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm font-medium">
                        {weekendData.code}
                      </span>
                      <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                        {weekendData.no_of_rooms} Rooms
                      </span>
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                        {weekendData.total_people} Guests
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-gray-600">Total Weekend Package Cost</p>
                    <p className="text-sm text-gray-500 mt-1">₹{weekendData.price_per_room}/room</p>
                  </div>
                </div>
              </div>
              
              {/* Guest Details Summary */}
              {weekendData.guests && weekendData.guests.length > 0 && (
                <div className="bg-gray-50 rounded-xl p-5 mb-8">
                  <h2 className="text-lg font-semibold text-gray-800 mb-3">Guest Details</h2>
                  <div className="space-y-4">
                    {/* Adults Section */}
                    {weekendData.no_of_adults > 0 && (
                      <div>
                        <h3 className="font-medium text-blue-800 mb-2">Adults ({weekendData.no_of_adults})</h3>
                        {weekendData.guests.filter(g => g.type === 'adult').map((guest, idx) => (
                          <div key={idx} className="border-b border-gray-200 pb-2 mb-2 last:border-0">
                            <p className="font-medium">{guest.name || `Adult ${idx + 1}`}</p>
                            <p className="text-sm text-gray-600">
                              Age: {guest.age} | Cell: {guest.cell || 'N/A'} | Email: {guest.email || 'N/A'}
                            </p>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Children Section */}
                    {weekendData.no_of_children > 0 && (
                      <div>
                        <h3 className="font-medium text-green-800 mb-2">Children ({weekendData.no_of_children})</h3>
                        {weekendData.guests.filter(g => g.type === 'child').map((guest, idx) => (
                          <div key={idx} className="border-b border-gray-200 pb-2 mb-2 last:border-0">
                            <p className="font-medium">{guest.name || `Child ${idx + 1}`}</p>
                            <p className="text-sm text-gray-600">
                              Age: {guest.age} | Cell: {guest.cell || 'N/A'} | Email: {guest.email || 'N/A'}
                            </p>
                          </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                        ...
                        The remaining {formatPrice(balanceAmount)} ({100 - paymentPercentage}%) will be payable before the check-in date.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between items-center py-3 border-b">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{weekendData.no_of_rooms} rooms × ₹{weekendData.price_per_room}</p>
                  </div>
                  <span className="text-lg font-bold text-gray-900">{formatPrice(weekendData.no_of_rooms * weekendData.price_per_room)}</span>
                </div>
                
                <div className="flex justify-between items-center py-3 border-b">
                  <div>
                    <span className="text-gray-700 font-medium">Total Package Cost</span>
                    <p className="text-sm text-gray-500">Including all taxes & services</p>
                  </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
                ...
                <ul className="text-sm text-green-700 space-y-1">
                  <li>✓ Accommodation for {weekendData.no_of_rooms} room(s)</li>
                  <li>✓ Complimentary breakfast</li>
                  <li>✓ Wi-Fi access</li>
                  <li>✓ Parking facility</li>
                  <li>✓ Access to amenities</li>
                </ul>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">support@weekendgateway.com</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex justify-between mb-2">
                <span className="text-gray-700">Total Weekend Package Cost:</span>
...
export default CheckoutPageWeekend;