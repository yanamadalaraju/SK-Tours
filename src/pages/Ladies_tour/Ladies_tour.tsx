...
const [paginatedTours, setPaginatedTours] = useState([]);
  ...
  // ---------- Fetch full tour details including EMI for each ladies special tour ----------
  useEffect(() => {
      ...
      try {
        ...
        const results = await Promise.all(
          allTours.map(async (tour) => {
            try {
              // Note: Using /full/ladiesspecial/ for ladies special tours
      ...
      // ✅ Extract departure months
          ...
          if (Array.isArray(data.departures)) {
            data.departures.forEach((dep: any) => {
              ...
              // Get EMI price from basic_details for ladies special tours
              ...
              console.error(
                "Error fetching ladies special tour details for",
                tour.tour_id,
                err
              );
              return { 
  ...
  // Function to get tours for current state (ladies special)
  const getCurrentStateTours = () => {
    ...
    console.log("Filtering ladies special tours for state:", selectedState);
    console.log("Total tours to filter:", allTours.length);
    
    // Filter by state AND tour_type = "ladiesspecial" (case-insensitive)
    ...
    console.log("Filtered ladies special tour IDs:", filtered.map(t => t.tour_id));
    return filtered;
  };

  // Function to format tours with dynamic EMI for ladies special
  const formatTours = (tours: any[]) => {
    console.log("Formatting", tours.length, "ladies special tours");
    
    return tours.map((tour) => {
      console.log("Formatting ladies special tour:", {
        id: tour.tour_id,
        code: tour.tour_code,
        type: tour.tour_type,
  ...
  useEffect(() => {
    ...
    // Only format if we have EMI data and images loaded for ladies special
    if (Object.keys(tourEmiData).length === 0 || Object.keys(tourImages).length === 0) {
      console.log("Waiting for ladies special tour details to load...");
      return;
    }
    ...
    console.log("Ladies special tour EMI data available for:", Object.keys(tourEmiData).length, "tours");
    
    const currentStateTours = getCurrentStateTours();
    console.log("Ladies special tours for current state:", currentStateTours.length);
    
    const formatted = formatTours(currentStateTours);
    console.log("Formatted ladies special tours:", formatted);
          ...
          <aside className="lg:w-80">
            <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-2xl shadow-lg p-6 border border-blue-200 sticky top-24">
              <div className="flex justify-between items-center mb-6 bg-white p-2 rounded-lg border border-black">
                <h2 className="text-2xl font-bold text-[#2E4D98]">Ladies SPL Tours</h2>
                <button 
                ...
                <div key={place} className="flex items-center gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={() => {
                      ...
                      navigate(`/intl-ladies_tours/${encodedDestination}`);
                    }}
