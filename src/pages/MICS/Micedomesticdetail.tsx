...
const Micedomesticdetail = () => {
 ...
 const processExhibitionData = (apiData: any, imagesData: any) => {
  const {
    mice_city,
    tours,
    itineraries,
  ...
  const getImages = () => {
    // Fix: Properly check if imagesData exists and has items
    if (imagesData && Array.isArray(imagesData) && imagesData.length > 0) {
      // Map through imagesData and extract the url from each object
      const imageUrls = imagesData.map((img: any) => img.url);
      console.log('Image URLs extracted:', imageUrls); // Debug log
      return imageUrls;
    }
 ...
 const processTourCost = () => {
  // Process tour_costs from API data
  if (tour_costs && tour_costs.length > 0) {
    const tableData = tour_costs.map((cost: any) => ({
      passenger: cost.pax || 'N/A',
      standard: formatPriceExhibition(cost.standard_hotel),
      deluxe: formatPriceExhibition(cost.deluxe_hotel),
      executive: formatPriceExhibition(cost.executive_hotel)
    }));
    return {
      tableData: tableData,
  ...
  return {
    ...
    const processTransport = () => {
      if (transports && transports.length > 0) {
        return {
          tableData: transports.map((transport: any, index: number) => ({
...
const fetchExhibitionDetails = async () => {
  try {
    ...
    // Fetch main exhibition details
    const response = await fetch(`${BASE_URL}/api/mice/domestic-details/${tourId}`);
    ...
    let imagesData = [];
    try {
      const imagesResponse = await fetch(`${BASE_URL}/api/mice/mice-images/${tourId}`);
    ...
    const processedData = processExhibitionData(result.data, imagesData);
    console.log('Processed data:', processedData);
    console.log('Images in processed data:', processedData.images);
...
useEffect(() => {
  ...
  const departuresByMonth = React.useMemo(() => {
    ...
    tour.departures.data.forEach((dep: any) => {
                  ...
                  <div className="min-w-[800px] lg:min-w-0">
                    <div className="grid grid-cols-8 bg-[#E8F0FF] border-b border-black">
                      ...
                      <div className="col-span-6 border-r border-white bg-[#2E3a8a] px-2 lg:px-4 py-2 lg:py-3">
                        <h3 className="font-bold text-white text-left text-sm lg:text-lg">Mice Name</h3>
                      </div>
...
export default Micedomesticdetail;