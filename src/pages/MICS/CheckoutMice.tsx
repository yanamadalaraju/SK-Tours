// src/pages/CheckoutMice.jsx - Fixed with correct API endpoints
...
const CheckoutMice = () => {
  ...
  // Get MICE data from navigation state or localStorage
  const [miceData, setMiceData] = useState(null);
  ...
  // Initialize MICE data
  useEffect(() => {
    if (location.state?.mice) {
      const mice = location.state.mice;
      setMiceData(mice);
      
      const totalMiceCost = mice.total_price_value || mice.priceValue || parsePrice(mice.price);
      setCustomPaymentAmount(totalMiceCost.toString());
      setPaymentType('full');
      
      setLoading(false);
    } else {
      const savedMice = localStorage.getItem('selectedMice');
      if (savedMice) {
        const parsedMice = JSON.parse(savedMice);
        setMiceData(parsedMice);
        
        const totalMiceCost = parsedMice.total_price_value || parsedMice.priceValue || parsePrice(parsedMice.price);
        setCustomPaymentAmount(totalMiceCost.toString());
        setPaymentType('full');
      }
      setLoading(false);
  ...
  const handleCustomAmountChange = (value) => {
    ...
    if (miceData) {
      const totalMiceCost = getTotalMiceCost();
      const enteredAmount = parseFloat(value) || 0;
      
      if (enteredAmount >= totalMiceCost) {
        setPaymentType('full');
        setIsPartialPayment(false);
      } else if (enteredAmount > 0 && enteredAmount < totalMiceCost) {
  ...
  const getTotalMiceCost = () => {
    if (!miceData) return 0;
    return miceData.total_price_value || miceData.priceValue || parsePrice(miceData.price);
  };
  ...
  const getCurrentPaymentAmount = () => {
    ...
    return getTotalMiceCost();
  };

  const validateCustomAmount = () => {
    if (!miceData) return false;
    
    const totalMiceCost = getTotalMiceCost();
    const minAmount = 1;
    const maxAmount = totalMiceCost;
    ...
    if (enteredAmount > maxAmount) {
      alert(`Amount cannot exceed total MICE cost of ${formatPrice(maxAmount)}`);
      return false;
    }
  ...
  const calculatePercentage = (amount) => {
    const total = getTotalMiceCost();
    return total > 0 ? Math.round((amount / total) * 100) : 0;
  };

  const getMiceDisplayName = () => {
    if (!miceData) return '';
    if (miceData.state_name) {
      return `${miceData.city_name} MICE Package`;
    } else if (miceData.country_name) {
      return `${miceData.country_name} MICE Package`;
    }
    return miceData.title || miceData.city_name || 'MICE Package';
  };

  const getDurationDisplay = () => {
    if (!miceData) return '';
    const days = miceData.duration_days;
    if (days) {
      return `${days - 1}N/${days}D`;
    }
    return miceData.duration || 'N/A';
  };

  // Handle payment - FIXED: Use correct API endpoint (same as tours)
  const handlePhonePePayment = async (e) => {
    ...
    const isFullPayment = paymentAmount >= totalMiceCost;
    ...
    try {
      const paymentDescription = isFullPayment 
        ? `Full Payment for ${getMiceDisplayName()}`
        : `${paymentPercentage}% Partial Payment for ${getMiceDisplayName()}`;
      
      console.log('Payment processing:', {
        amount: paymentAmount,
        totalMiceCost: totalMiceCost,
      ...
      const checkoutResponse = await axios.post(
        ...
        {
          // Map MICE data to match the table structure
          tour_id: miceData.id || '',
          tour_code: miceData.code || miceData.mice_code || `MICE_${miceData.id}`,
          tour_title: getMiceDisplayName(),
          tour_duration: getDurationDisplay(),
          tour_locations: miceData.city_name || miceData.country_name || '',
          tour_image_url: miceData.image || '',
          
          // Price details
          total_tour_cost: totalMiceCost,
          advance_percentage: paymentPercentage,
          advance_amount: paymentAmount,
          emi_price: miceData.emi_price || miceData.emiPriceValue || 0,
          ...
          source_page: 'mice-packages',
          source: 'mice',  // ✅ Set source to 'mice'
      ...
      const merchantOrderId = `MICE_${checkoutId}_${Date.now()}`;
      ...
      if (paymentResponse.data.success) {
        ...
        const bookingData = {
          mice: {
            ...
            display_name: getMiceDisplayName(),
        ...
        localStorage.setItem('currentMiceBooking', JSON.stringify(bookingData));
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-gray-50">
          <div className="bg-white p-8 rounded-2xl shadow-lg text-center max-w-md">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">No MICE Package Selected</h2>
            <p className="text-gray-600 mb-6">Please select a MICE package to proceed with booking.</p>
            <Button 
              onClick={() => navigate('/mice')} 
              className="bg-[#2E4D98] hover:bg-[#2E4D98]/90 px-8 py-6 text-lg"
            >
              Browse MICE Packages
            </Button>
          </div>
        </div>
        <Footer />
  ...
  const paymentPercentage = calculatePercentage(paymentAmount);
  const balanceAmount = totalMiceCost - paymentAmount;
  const isFullPayment = paymentAmount >= totalMiceCost;
  const isDomestic = miceData.state_name !== undefined;
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h1 className="text-3xl font-bold text-gray-800 mb-2">Complete Your MICE Booking</h1>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-5 mb-8 border border-blue-100">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
                  ...
                  MICE Package Summary
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="font-bold text-lg text-gray-800 mb-2">{getMiceDisplayName()}</h3>
                    <p className="text-gray-600 mb-3">
                      {isDomestic 
                        ? `${miceData.city_name}, ${miceData.state_name}`
                        : miceData.country_name}
                    </p>
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                        {isDomestic ? 'Domestic MICE' : 'International MICE'}
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold text-gray-900 mb-1">
                      {formatPrice(totalMiceCost)}
                    </div>
                    <p className="text-gray-600">Total Package Cost</p>
                    {miceData.emi_price && (
                      <p className="text-sm text-gray-500 mt-1">EMI: {formatPrice(miceData.emi_price)}/person/month</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5 mb-8">
                ...
                <div className="mb-4">
                  <div className={`flex items-center justify-between p-4 border rounded-lg cursor-pointer transition-all ${paymentType === 'full' ? 'border-[#2E4D98] bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-gray-900">{formatPrice(totalMiceCost)}</div>
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                        ...
                        The remaining {formatPrice(balanceAmount)} ({100 - paymentPercentage}%) will be payable before the event departure date.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <form onSubmit={handlePhonePePayment} className="space-y-8">
                ...
                <div className="flex items-start space-x-3 p-4 bg-gray-50 rounded-lg">
                  ...
                  <Label htmlFor="terms" className="cursor-pointer text-sm">
                    ...
                    . {isPartialPayment && `I understand that I'm paying ${paymentPercentage}% now and the remaining amount must be paid before the event departure date.`} <span className="text-red-500">*</span>
                  </Label>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between items-center py-3 border-b">
                  ...
                  <span className="text-lg font-bold text-gray-900">{formatPrice(totalMiceCost)}</span>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 mb-2">
                      Our MICE experts are available 24/7
                    </p>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex justify-between mb-2">
                <span className="text-gray-700">Total Package Cost:</span>
                <span className="font-bold">{formatPrice(totalMiceCost)}</span>
              </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder={`Enter amount (max: ${formatPrice(totalMiceCost)})`}
              ...
              <div className="flex justify-between text-sm text-gray-500 mt-2">
                <span>Min: ₹1</span>
                <span>Max: {formatPrice(totalMiceCost)}</span>
              </div>
            </div>
                  ...
                  {!isFullPayment && (
                    <div className="mt-2">
                      ...
                      <div className="text-lg font-bold text-gray-800">
                        {formatPrice(totalMiceCost - parseFloat(customPaymentAmount))}
                      </div>
                    </div>
                  )}
                </div>
