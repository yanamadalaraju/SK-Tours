// src/pages/CheckoutPageBungalow.tsx
...
const CheckoutPageBungalow: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  const [bungalowData, setBungalowData] = useState<BungalowData | null>(null);
  ...
  // Initialize bungalow data
  useEffect(() => {
    if (location.state?.bungalow) {
      const bungalow = location.state.bungalow;
      setBungalowData(bungalow);
      
      // Pre-fill form with bungalow contact info
      setFormData(prev => ({
        ...prev,
        email: bungalow.email_id || '',
        phone: bungalow.cell_no || '',
        address: bungalow.address || '',
        city: bungalow.city || '',
        state: bungalow.state || '',
        pincode: bungalow.pin_code || '',
        country: bungalow.country || 'India',
        firstName: bungalow.contact_person?.split(' ')[0] || '',
        lastName: bungalow.contact_person?.split(' ')[1] || '',
      }));
      
      const totalCost = bungalow.total_price_value || 5000;
      setCustomPaymentAmount(totalCost.toString());
      setPaymentType('full');
      setLoading(false);
    } else {
      const savedBungalow = localStorage.getItem('selectedBungalow');
      if (savedBungalow) {
        const parsedBungalow = JSON.parse(savedBungalow);
        setBungalowData(parsedBungalow);
        
        setFormData(prev => ({
          ...prev,
          email: parsedBungalow.email_id || '',
          phone: parsedBungalow.cell_no || '',
          address: parsedBungalow.address || '',
          city: parsedBungalow.city || '',
          state: parsedBungalow.state || '',
          pincode: parsedBungalow.pin_code || '',
          firstName: parsedBungalow.contact_person?.split(' ')[0] || '',
          lastName: parsedBungalow.contact_person?.split(' ')[1] || '',
        }));
        
        const totalCost = parsedBungalow.total_price_value || 5000;
        setCustomPaymentAmount(totalCost.toString());
        setPaymentType('full');
      }
      setLoading(false);
  ...
  const handleCustomAmountChange = (value: string) => {
    ...
    if (bungalowData) {
      const totalCost = getTotalBungalowCost();
  ...
  const getTotalBungalowCost = () => {
    if (!bungalowData) return 0;
    return bungalowData.total_price_value || 5000;
  };
  ...
  const getCurrentPaymentAmount = () => {
    ...
    return getTotalBungalowCost();
  };

  const validateCustomAmount = () => {
    if (!bungalowData) return false;
    ...
    if (enteredAmount > maxAmount) {
      alert(`Amount cannot exceed total bungalow cost of ${formatPrice(maxAmount)}`);
      return false;
    }
  ...
  const calculatePercentage = (amount: number) => {
    const total = getTotalBungalowCost();
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      const paymentDescription = isFullPayment 
        ? `Full Payment for ${bungalowData?.title || bungalowData?.code}`
        : `${paymentPercentage}% Partial Payment for ${bungalowData?.title || bungalowData?.code}`;
      
      // FIXED: Map bungalow fields to backend expected fields
      const checkoutPayload = {
        // Backend required fields
        tour_id: bungalowData?.id || bungalowData?.code || 'bungalow_001',
        total_tour_cost: totalCost,
        advance_amount: paymentAmount,
        
        // Additional bungalow specific fields
        bungalow_id: bungalowData?.id || '',
        bungalow_code: bungalowData?.code || '',
        bungalow_title: bungalowData?.title || '',
        bungalow_city: bungalowData?.city || '',
        bungalow_address: bungalowData?.address || '',
        ...
        // Guest details
        no_of_people: bungalowData?.no_of_people || 1,
        guests: JSON.stringify(bungalowData?.guests || []),
        
        payment_method: formData.paymentMethod,
        source_page: 'bungalow-booking',
        source: 'bungalows',
        terms_accepted: formData.termsAccepted,
        notes: paymentDescription,
        booking_type: 'bungalow'
      };
      ...
      const merchantOrderId = `BUNG_${checkoutId}_${Date.now()}`;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-2xl shadow-lg text-center max-w-md">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">No Bungalow Selected</h2>
          <p className="text-gray-600 mb-6">Please select a bungalow to proceed with booking.</p>
          <Button 
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h1 className="text-3xl font-bold text-gray-800 mb-2">Complete Your Bungalow Booking</h1>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-5 mb-8 border border-blue-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="font-bold text-lg text-gray-800 mb-2">{bungalowData.title}</h3>
                    <p className="text-gray-600 mb-3">{bungalowData.city}, {bungalowData.state}</p>
                    <div className="flex items-center gap-4">
                      <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                        {bungalowData.code}
                      </span>
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                        {bungalowData.no_of_people} Guests
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-gray-600">Total Bungalow Cost</p>
                  </div>
                </div>
              </div>
              
              {/* Guest Details Summary */}
              {bungalowData.guests && bungalowData.guests.length > 0 && (
                <div className="bg-gray-50 rounded-xl p-5 mb-8">
                  <h2 className="text-lg font-semibold text-gray-800 mb-3">Guest Details</h2>
                  <div className="space-y-3">
                    {bungalowData.guests.map((guest, idx) => (
                      <div key={idx} className="border-b border-gray-200 pb-2 last:border-0">
                        <p className="font-medium">{guest.name}</p>
                        <p className="text-sm text-gray-600">Age: {guest.age} | Cell: {guest.cell} | Email: {guest.email}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between items-center py-3 border-b">
                  <div>
                    <span className="text-gray-700 font-medium">Total Bungalow Cost</span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">
                      Free cancellation up to 48 hours before check-in. Your payment is fully refundable.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">support@bungalowstay.com</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex justify-between mb-2">
                <span className="text-gray-700">Total Bungalow Cost:</span>
...
export default CheckoutPageBungalow;