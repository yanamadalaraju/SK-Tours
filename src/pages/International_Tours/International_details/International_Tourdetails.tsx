...
const International_Tourdetails = () => {
  ...
  const isMonthInPast = (monthYearString) => {
    if (!monthYearString) return false;
    // monthYearString format: "AUG 2025"
    ...
    // If year is in the past, month is in the past
    if (year < currentYear) return true;
    
    // If year is in the future, month is not in the past
  ...
  const processTourData = (apiData) => {
    ...
    // Process departures with past date filtering
    const processDepartures = (departuresArray, tour_type) => {
      ...
      if (isGroupType) {
        // First, filter out past departure dates
        const filteredDepartures = departuresArray.filter((dep) => {
          const startDate = dep.start_date ? new Date(dep.start_date) : null;
          if (!startDate) return true;
          
          const today = new Date();
          today.setHours(0, 0, 0, 0);
          
          // Keep departures where start date is today or in the future
          return startDate >= today;
        });

        const departureItems = filteredDepartures.map((dep, index) => {
  ...
  // Reset selectedMonth if it no longer exists in availableMonths
  useEffect(() => {
    if (selectedMonth !== "ALL" && availableMonths.length > 0 && !availableMonths.includes(selectedMonth)) {
      setSelectedMonth("ALL");
    }
    // Also reset if there are no departures at all
  ...
  // Reset selectedCostMonth if it no longer exists
  useEffect(() => {
    if (selectedCostMonth && availableMonths.length > 0 && !availableMonths.includes(selectedCostMonth)) {
      setSelectedCostMonth("");
      setSelectedCostDate("");
    }
    if (availableMonths.length === 0 && selectedCostMonth) {
      setSelectedCostMonth("");
      setSelectedCostDate("");
    }
  }, [availableMonths, selectedCostMonth]);

  // Reset selectedCostDate if it no longer exists in the selected month
  useEffect(() => {
    if (selectedCostMonth && selectedCostDate) {
      const monthDates = departuresByMonth[selectedCostMonth] || [];
      const dateExists = monthDates.some(dep => dep.fromDate === selectedCostDate);
      if (!dateExists) {
        setSelectedCostDate("");
      }
    }
  }, [departuresByMonth, selectedCostMonth, selectedCostDate]);
                      ...
                      <div className="border-2 border-t-0 border-[#1e3a8a] rounded-b-lg w-full flex flex-col min-h-[400px] lg:min-h-[680px] max-h-[400px] lg:max-h-[780px] overflow-hidden">
                        <div className="flex-1 overflow-y-auto p-2 bg-[#FFEBEE] w-full">
                          <div className="flex flex-wrap gap-1 lg:gap-2 mb-2 overflow-x-auto pb-2">
                            {(() => {
                              // Use availableMonths from the map (already filtered by processDepartures)
          ...
          <ul className="space-y-0">
            {tour.bookingRemarks.map((remark, index) => (
              <li key={index} className="border-b-[1px] border-black last:border-b-0">
                <div className="p-1">
                  <span className="block text-black whitespace-pre-wrap break-normal text-sm lg:text-base text-justify [text-align-last:left]">
                    {remark}
                  </span>
                </div>
              </li>
  ...
  {/* RIGHT — Cancellation Policy Remarks */}
  <div className="bg-[#E8F0FF] rounded-lg overflow-hidden">
