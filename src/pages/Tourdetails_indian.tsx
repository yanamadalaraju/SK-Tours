...
const departureData: DepartureItem[] = [
  // APR 2026
  {
    month: "APR 2026",
    fromDay: "Friday",
...
const Tourdetails_indian = () => {
  ...
  // Add this state for departure date filtering
  const [selectedMonth, setSelectedMonth] = useState("ALL");

  // State for toggle table visibility
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  // Toggle function for showing/hiding table
  const toggleTable = (index: number) => {
    setOpenIndex(openIndex === index ? null : index);
  };

  // Filter departure data based on selected month
  const filteredData =
    selectedMonth === "ALL"
      ? departureData
      : departureData.filter((d) => d.month === selectedMonth);
  ...
  // Month & Date Dropdown States
  const [monthDropdownOpen, setMonthDropdownOpen] = useState(false);
  const [dateDropdownOpen, setDateDropdownOpen] = useState(false);
  const [selectedTourMonth, setSelectedTourMonth] = useState<string>("");
  const [selectedTourDate, setSelectedTourDate] = useState<number | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.month-dropdown-container') && !target.closest('.date-dropdown-container')) {
  ...
  const processTourData = (apiData: any) => {
    ...
    const processOptionalTours = (optionalToursArray: any[]) => {
      ...
      // Return empty array if no optional tours
      return [];
    };

    // Process EMI Options
    const processEMIOptions = (emiOptionsObj: any) => {
      if (emiOptionsObj && emiOptionsObj.options && emiOptionsObj.options.length > 0) {
        return {
          loanAmount: formatPrice(emiOptionsObj.loan_amount || '0'),
          options: emiOptionsObj.options.map((option: any) => ({
      ...
      // Default EMI options if none provided
      const basePrice = parseFloat(basic_details.base_price_adult) || 15000;
      const defaultOptions = [6, 12, 18, 24, 30, 36, 48].map(months => ({
        particulars: 'Per Month Payment',
        months: months,
        emi: formatPrice(Math.round(basePrice / months))
      }));
      
      return {
        loanAmount: formatPrice(basePrice),
    ...
    return {
      ...
      departureDescriptions: processDepartureDescriptions(departures),
      inclusionExclusion: {
  ...
  const clearAllFilters = () => {
    ...
    setSelectedMonth("ALL"); // Reset departure month filter
    setOpenIndex(null); // Close any open tables
  };
...
const tabId = label
      ...
      {tab}
                      ...
                      <div className="space-y-4 w-full">
                        {filteredData.map((item, index) => (
                          <div key={index} className="border border-2 border-black  p-4 bg-white space-y-4">
                            {/* MAIN CARD */}
  ...
  <div className="bg-[#E8F0FF] rounded-lg p-1">
    ...
    {/* Month & Date Selection Card with integrated dropdowns and table */}
    <div className="bg-white border-2 border-[#1e3a8a]  rounded-b-lg  p-4 mb-1.5 shadow-sm">
      {/* Main Row with Month and Date */}
          ...
          <div className="relative">
            <button
              ...
              onClick={() => {
                ...
                setDateDropdownOpen(false); // Close date dropdown when opening month
              }}
              className="w-full bg-white border-2 border-[#1e3a8a]  px-4 py-3 text-left flex justify-between items-center hover:bg-gray-50"
            >
              <span className="text-gray-700">{selectedTourMonth || "Select Month"}</span>
              <svg
            ...
            {/* Month Dropdown - Positioned to the right of the button */}
            {monthDropdownOpen && (
              <div className="absolute z-10 left-0 right-0 mt-1 bg-white border-2 border-[#1e3a8a] rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setSelectedTourMonth(month);
                      setMonthDropdownOpen(false);
                    }}
                    className="w-full px-4 py-3 text-left hover:bg-[#E8F0FF] border-b border-gray-100 last:border-b-0"
                  >
          ...
          <div className="relative">
            <button
              ...
              onClick={() => {
                ...
                setMonthDropdownOpen(false); // Close month dropdown when opening date
              }}
              className="w-full bg-white border-2 border-[#1e3a8a] px-4 py-3 text-left flex justify-between items-center hover:bg-gray-50"
            >
              <span className="text-gray-700">
                {selectedTourDate ? `Date ${selectedTourDate}` : "Select Date"}
              </span>
              <svg
                className={`w-5 h-5 text-gray-500 transition-transform ${
            ...
            {/* Date Dropdown - Positioned to the right of the button */}
            {dateDropdownOpen && (
              <div className="absolute z-10 left-0 right-0 mt-1 bg-white border-2 border-[#1e3a8a] rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {/* Dates from 1-31 */}
      ...
      {/* Table inside the same card - Shows when both month and date are selected */}
      {selectedTourMonth && selectedTourDate && (
        <div className="mt-4 border-2 border-black overflow-hidden animate-fadeIn">
          {/* HEADER */}
          <div className="grid grid-cols-4 bg-[#0A1D4A] text-white font-semibold text-center">
            <div className="p-3 border-r-2 border-white">Particulars - Tour Cost</div>
            <div className="p-3 border-r-2 border-white">3 Star</div>
            <div className="p-3 border-r-2 border-white">4 Star</div>
            <div className="p-3">5 Star</div>
          </div>

          {/* ROWS */}
          {/* You can replace this with dynamic data based on selected month/date */}
          {[
            { particular: "Adult", star3: "₹12,000", star4: "₹15,000", star5: "₹18,000" },
            { particular: "Child (With Bed)", star3: "₹10,000", star4: "₹12,000", star5: "₹15,000" },
            { particular: "Child (No Bed)", star3: "₹8,000", star4: "₹10,000", star5: "₹12,000" },
          ].map((row, i) => (
            <div
              key={i}
              className={`grid grid-cols-4 text-center border-b-2 border-black ${
            ...
            >
              <div className="p-3 border-r-2 border-black font-medium">
                {row.particular}
              </div>
              <div className="p-3 border-r-2 border-black">
                {row.star3}
              </div>
              <div className="p-3 border-r-2 border-black font-semibold text-green-700">
                {row.star4}
              </div>
              <div className="p-3">
                {row.star5}
    ...
    {/* Rest of your existing Tour Cost content */}
              ...
              {tour.optionalTours && tour.optionalTours.length > 0 ? (
                tour.optionalTours.map((optTour: any, index: number) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-[#FFEBEE]" : "bg-[#FFEBEE]/80"}>
                    ...
                    <td className="border border-black px-4 py-2 border-l-0">
                      {optTour.adultPrice}
                    </td>
                    <td className="border border-black px-4 py-2 border-l-0">
                      {optTour.childPrice}
                ...
                // Show empty rows if no optional tours
                <>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody className="border-2 border-[#1e3a8a] border-t-0">
              {tour.emiOptions && tour.emiOptions.options && tour.emiOptions.options.length > 0 ? (
                ...
                // Default EMI options if none available
                <>
                  <tr className="bg-[#FFEBEE]">
                    <td className="border border-black px-4 py-2 font-bold text-base">Per Month Payment</td>
                    <td className="border border-black px-4 py-2 border-l-0 text-center">
                      {tour.price}
                    </td>
                    <td className="border border-black px-4 py-2 border-l-0 text-center">6</td>
                    <td className="border border-black px-4 py-2 border-l-0 text-center">
                      {formatPrice(parseFloat(tour.price.replace('₹', '').replace(',', '')) / 6)}
                    </td>
                  </tr>
                  <tr className="bg-[#FFEBEE]/80">
                    <td className="border border-black px-4 py-2 font-bold text-base">Per Month Payment</td>
                    <td className="border border-black px-4 py-2 border-l-0 border-t-0 text-center">
                      {tour.price}
                    </td>
                    <td className="border border-black px-4 py-2 border-l-0 border-t-0 text-center">12</td>
                    <td className="border border-black px-4 py-2 border-l-0 border-t-0 text-center">
                      {formatPrice(parseFloat(tour.price.replace('₹', '').replace(',', '')) / 12)}
                    </td>
                  </tr>
                </>
              )}
                      ...
                      <div className="min-h-[300px] max-h-[400px] overflow-y-auto p-1 bg-[#FFEBEE] w-full">
                        <div className="overflow-x-auto border shadow-sm">
                          <table className="w-full border-collapse table-fixed">
                            ...
                            {/* ===== TABLE BODY - 4 EMPTY ROWS USING LOOP ===== */}
                            <tbody>
...
export default Tourdetails_indian;