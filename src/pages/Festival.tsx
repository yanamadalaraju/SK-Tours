...
const stateHeroImages = {
  ...
  "Gujarat": "https://images.unsplash.com/phone-1587132135058-bc3c3dcfd4db?w=1200&q=80",
...
const [paginatedTours, setPaginatedTours] = useState([]);
  ...
  const [allFormattedTours, setAllFormattedTours] = useState<any[]>([]);
          ...
          allTours.map(async (tour) => {
            try {
              const res = await fetch(
                `${BASE_URL}/api/tours/tour/full/festival/${tour.tour_id}`
              );
              const data = await res.json();
  ...
  const getCurrentStateTours = () => {
    ...
    // Filter by state AND tour_type = "festival" (case-insensitive)
    const filtered = allTours.filter((tour) => {
      const stateMatch = tour.primary_destination_name?.toLowerCase() === selectedState.toLowerCase();
      const typeMatch = tour.tour_type?.toLowerCase() === "festival";
  ...
  useEffect(() => {
    ...
    const allfestival = allTours.filter(t => t.tour_type?.toLowerCase() === "festival");
    const allFormatted = formatTours(allfestival);
    ...
    const currentStateTours = allTours.filter(tour => {
      return (
        ...
        tour.tour_type?.toLowerCase() === "festival"
      );
    });
            ...
            <aside className="lg:w-80">
              <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-2xl shadow-lg p-6 border border-blue-200 sticky top-24">
                <div className="flex justify-between items-center mb-6 bg-white p-2 rounded-lg border border-black">
                  <h2 className="text-2xl font-bold text-[#2E4D98]">festival Tours</h2>
                  <button
        ...
        <div key={place} className="flex items-center gap-3 cursor-pointer">
          <Checkbox
            ...
            onCheckedChange={(checked) => {
              if (checked) {
                clearAllFilters();
                navigate(`/festival/${encodeURIComponent(place)}`);
              }
            }}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Showing {filteredTours.length} of {formattedTours.length} festival tours • Best prices guaranteed
                  </p>
                </div>
              </div>
{filteredTours.length === 0 ? (
  <div className="text-center py-12">
    <h3 className="text-xl font-semibold text-gray-600">No festival tours found for the selected filters</h3>
