...
const [paginatedTours, setPaginatedTours] = useState([]);
          ...
          allTours.map(async (tour) => {
            try {
              const res = await fetch(
                `${BASE_URL}/api/tours/tour/full/sports/${tour.tour_id}`
              );
              const data = await res.json();
  ...
  const getCurrentStateTours = () => {
    ...
    // Filter by state AND tour_type = "sports" (case-insensitive)
    const filtered = allTours.filter((tour) => {
      const stateMatch = tour.primary_destination_name?.toLowerCase() === selectedState.toLowerCase();
      const typeMatch = tour.tour_type?.toLowerCase() === "sports";
  ...
  useEffect(() => {
    ...
    const allsports = allTours.filter(t => t.tour_type?.toLowerCase() === "sports");
    const allFormatted = formatTours(allsports);
    ...
    const currentStateTours = allTours.filter(tour => {
      return (
        ...
        tour.tour_type?.toLowerCase() === "sports"
      );
    });
            ...
            <aside className="lg:w-80">
              <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-2xl shadow-lg p-6 border border-blue-200 sticky top-24">
                <div className="flex justify-between items-center mb-6 bg-white p-2 rounded-lg border border-black">
                  <h2 className="text-2xl font-bold text-[#2E4D98]">sports Tours</h2>
                  <button
        ...
        <div key={place} className="flex items-center gap-3 cursor-pointer">
          <Checkbox
            ...
            onCheckedChange={(checked) => {
              if (checked) {
                clearAllFilters();
                navigate(`/sports/${encodeURIComponent(place)}`);
              }
            }}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Showing {filteredTours.length} of {formattedTours.length} sports tours • Best prices guaranteed
                  </p>
                </div>
              </div>
