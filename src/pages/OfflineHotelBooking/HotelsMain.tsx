...
const HotelCard = ({ hotel, onBookNow }: { hotel: Hotel; onBookNow: (hotel: Hotel) => void }) => {
  ...
  const tabContent = {
    ...
    facilities: (
      <div className="p-4">
        ...
        {hotel.hotel_facilities_description ? (<div className="text-sm text-gray-700">{hotel.hotel_facilities_description}</div>) : (
          <div className="grid grid-cols-2 gap-3">
    ...
    transfer: (
      <div className="p-4">
        ...
        {hotel.airport_transfers_description ? (<div className="text-sm text-gray-700">{hotel.airport_transfers_description}</div>) : (
          <table className="w-full text-sm"><tbody><tr className="border-b"><td className="py-2 font-medium">Nearest Airport</td><td className="py-2">Goa International Airport (GOI)</td></tr><tr className="border-b"><td className="py-2 font-medium">Distance</td><td className="py-2">36 km</td></tr><tr className="border-b"><td className="py-2 font-medium">Transfer Service</td><td className="py-2 text-green-600">✓ Available on request</td></tr><tr className="border-b"><td className="py-2 font-medium">Transfer Cost</td><td className="py-2">₹2,500 - ₹3,500 per vehicle</td></tr></tbody></table>
        )}
      </div>
    ),
    meal: (
      <div className="p-4">
        ...
        {hotel.meal_plan_description ? (<div className="text-sm text-gray-700">{hotel.meal_plan_description}</div>) : (
          <div className="space-y-2">{[{ plan: 'Room only (EP)', price: 'Included' }, { plan: 'Breakfast (CP)', price: '+₹800' }, { plan: 'Breakfast + Dinner (MAP)', price: '+₹1,500' }, { plan: 'All meals (AP)', price: '+₹2,200' }].map((meal, idx) => (<div key={idx} className="flex justify-between text-sm border-b pb-2"><span>{meal.plan}</span><span className="font-medium">{meal.price}</span></div>))}</div>
        )}
      </div>
    ),
    taxes: (
      <div className="p-4">
        ...
        {hotel.taxes_description ? (<div className="text-sm text-gray-700">{hotel.taxes_description}</div>) : (
          <div className="space-y-2">{[{ label: 'Base Price', amount: `₹${formatPrice(hotel.price)}` }, { label: 'SGST (9%)', amount: `₹${Math.round(Number(hotel.price) * 0.09).toLocaleString()}` }, { label: 'CGST (9%)', amount: `₹${Math.round(Number(hotel.price) * 0.09).toLocaleString()}` }, { label: 'Service Charge', amount: hotel.taxes ? `₹${formatPrice(hotel.taxes)}` : '₹315' }].map((item, idx) => (<div key={idx} className="flex justify-between text-sm border-b pb-2"><span>{item.label}</span><span className="font-medium">{item.amount}</span></div>))}<div className="flex justify-between text-sm font-bold pt-2"><span>Total per night</span><span>₹{(Number(hotel.price) + Math.round(Number(hotel.price) * 0.18) + (Number(hotel.taxes) || 315)).toLocaleString()}</span></div></div>
        )}
      </div>
    )
  };
      ...
      <div className="flex flex-col lg:flex-row">
        <div className="lg:w-64 mr-6 flex-shrink-0 mb-4 lg:mb-0">
          <div className="bg-gradient-to-br from-blue-400 to-blue-600 rounded-lg h-48 w-full mb-3 flex items-center justify-center overflow-hidden">
            {mainImageUrl ? (<img src={mainImageUrl} alt={hotel.hotel_name} className="w-full h-full object-cover" onError={() => setImageError(true)} />) : (<div className="text-white text-center"><div className="text-4xl mb-2">🏨</div><p className="font-bold text-lg">{hotel.hotel_name?.split(' ').slice(0, 2).join(' ')}</p><p className="text-sm opacity-90">Hotel</p></div>)}
          </div>
          {hotel.free_stay_for_kids && (<div className="flex items-center text-sm text-green-600 font-medium"><div className="w-2 h-2 bg-green-500 rounded-full mr-2"></div>Free stay for kids</div>)}
        </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-800 flex items-center flex-wrap gap-2">
                  {hotel.hotel_name}
                  <div className="flex">{[...Array(5)].map((_, i) => (<Star key={i} className={`w-4 h-4 ${i < (hotel.star_rating || 0) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} />))}</div>
                </h2>
                <div className="flex items-center text-gray-600 mt-1"><MapPin size={16} className="mr-1 flex-shrink-0" /><span className="text-sm">{hotel.hotel_location}</span></div>
              </div>
              <button className="text-blue-600 hover:text-blue-800 text-sm font-semibold flex items-center whitespace-nowrap">View on Map</button>
            </div>
          </div>

          {hotel.amenities && (<div className="mb-4"><ul className="space-y-2">{hotel.amenities.split(',').slice(0, 2).map((feature, idx) => (<li key={idx} className="flex items-start text-gray-700"><Check size={18} className="mr-2 flex-shrink-0 mt-0.5 text-green-600" /><span className="text-sm">{feature.trim()}</span></li>))}</ul></div>)}

          <div className="border-b border-gray-200 mb-4">
            <div className="flex flex-wrap gap-1">
              {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(activeTab === tab.id ? null : tab.id)} className={`px-3 py-2 text-xs font-medium transition-colors whitespace-nowrap rounded-t-lg ${activeTab === tab.id ? "border-b-2 border-orange-600 text-orange-600" : "text-gray-600 hover:text-gray-800"}`}>{tab.label}</button>))}
            </div>
          </div>

          {activeTab && (<div className="mb-4 bg-gray-50 rounded-lg">{tabContent[activeTab as keyof typeof tabContent]}</div>)}

          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mt-4 pt-4 border-t">
            <div className="mb-4 sm:mb-0">
              {hotel.rating && (<div className="flex items-center mb-3"><div className="bg-green-600 text-white px-3 py-1.5 rounded-l flex items-center"><span className="font-bold">{Number(hotel.rating).toFixed(1)}</span><span className="ml-1.5 text-sm font-medium">Very Good</span></div><div className="bg-green-50 text-green-700 px-3 py-1.5 rounded-r"><span className="text-sm">({hotel.total_ratings?.toLocaleString() || 0} Ratings)</span></div></div>)}
              <div className="flex items-center">{hotel.limited_time_sale && (<span className="bg-red-50 text-red-600 text-xs font-bold px-2 py-1 rounded mr-2">Limited Time Sale</span>)}<span className="text-sm text-gray-600">Per Night for 1 Room</span></div>
            </div>

            <div className="text-right w-full sm:w-auto">
              {hotel.original_price && Number(hotel.original_price) > Number(hotel.price) && (<div className="mb-1"><span className="text-gray-500 line-through text-sm">₹{formatPrice(hotel.original_price)}</span></div>)}
              <div className="flex items-baseline justify-end mb-2 flex-wrap"><span className="text-2xl font-bold text-gray-800">₹{formatPrice(hotel.price)}</span>{hotel.taxes && (<span className="text-gray-600 text-sm ml-2">+ ₹{formatPrice(hotel.taxes)} taxes</span>)}</div>
              <div className="space-y-2">
                <button className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-8 rounded-lg transition-colors w-full sm:w-auto">VIEW ALL</button>
                <button onClick={() => onBookNow(hotel)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition-colors w-full sm:w-auto">BOOK NOW</button>
                {hotel.login_to_book && (<p className="text-blue-600 text-sm font-semibold">Login to Book Now & Pay Later!</p>)}
              </div>
            </div>
          </div>
        </div>
...
const HotelSearchMain = () => {
  ...
  const filterHotels = () => {
    ...
    return hotels.filter(hotel => {
      ...
      const locationMatch = hotelCity.includes(searchLocation) || hotelLocation.includes(searchLocation);
      ...
      let guestMatch = true;
      if (travellers) {
        if (hotel.rooms && hotel.rooms < travellers.rooms) guestMatch = false;
        if (hotel.adults && hotel.adults < travellers.adults) guestMatch = false;
        if (hotel.children !== undefined && hotel.children < travellers.children) guestMatch = false;
      }
  ...
  const handleSearch = async () => {
    ...
    const newSearchData = { location, checkIn, checkOut, travellers, totalHotels: filtered.length };
    setSearchData(newSearchData);
    setShowResults(true);
    setTimeout(() => {
      window.scrollTo({ top: document.getElementById('hotel-results')?.offsetTop || 0, behavior: 'smooth' });
    }, 100);
  };

  const handleBookNow = (hotel: Hotel) => {
    // Prepare hotel data for checkout
    const hotelForCheckout = {
      ...
      nights: checkOut && checkIn ? Math.ceil((checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60 * 24)) : 1,
      total_price_value: calculateTotalCost(hotel)
    };
    localStorage.setItem('selectedHotel', JSON.stringify(hotelForCheckout));
    navigate('/checkout-hotels', { state: { hotel: hotelForCheckout } });
  };

  const calculateTotalCost = (hotel: Hotel) => {
  ...
  const parsePrice = (price: string | number | undefined) => {
    if (!price) return 0;
    const numericString = price.toString().replace(/[₹$,]/g, '').replace(/\s+/g, '').trim();
    return parseFloat(numericString) || 0;
  };

  const handleLocationSelect = (city: string) => { setLocation(city); setAutoOpenCheckIn(true); };
  const handleCheckInSelect = (date: Date) => { setCheckIn(date); setAutoOpenCheckOut(true); };
  const handleCheckOutSelect = (date: Date) => { setCheckOut(date); setAutoOpenGuests(true); };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-blue-50/70 to-white">
      ...
      <div className="flex-grow">
        <div className="pt-28 pb-28 relative bg-gradient-to-r from-blue-200/90 to-blue-300/90" style={{ backgroundImage: `linear-gradient(to right, rgba(74, 94, 149, 0.45), rgba(30, 64, 175, 0.49)), url('https://images.unsplash.com/photo-1566073771259-6a8506099945?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'overlay' }}>
          <div className="absolute inset-0 bg-gradient-to-r from-blue-900/80 to-blue-800/80"></div>
          <div className="max-w-7xl mx-auto px-4 relative z-10"><div className="flex items-center gap-3 mb-8"><Hotel className="w-8 h-8 text-white" /><h1 className="text-2xl font-bold text-white">Hotel Booking</h1></div></div>
        </div>
            ...
            <div className="flex flex-col md:flex-row">
              <div className="flex-1 grid grid-cols-1 md:grid-cols-4">
                <div className="relative border-b md:border-b-0 md:border-r border-gray-200">
                  <button onClick={() => { setShowLocationDropdown(!showLocationDropdown); setShowCheckInDropdown(false); setShowCheckOutDropdown(false); setShowGuestsDropdown(false); }} className="w-full h-full text-left px-4 py-3 bg-white hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0"><MapPin className="w-4 h-4 text-blue-600" /></div><div className="flex-1 min-w-0"><p className="text-xs text-gray-500 uppercase tracking-wide">CITY / HOTEL</p><p className="font-semibold text-gray-900 truncate">{location}</p><p className="text-xs text-gray-500 truncate">India</p></div><ChevronDown className="w-4 h-4 text-gray-500 shrink-0" /></div>
                  </button>
                  {showLocationDropdown && (<div className="absolute top-full left-0 mt-2"><LocationDropdown onClose={handleLocationClose} onSelectCity={handleLocationSelect} autoOpen={autoOpenCheckIn} /></div>)}
                </div>

                <div className="relative border-b md:border-b-0 md:border-r border-gray-200">
                  <button onClick={() => { setShowLocationDropdown(false); setShowCheckInDropdown(!showCheckInDropdown); setShowCheckOutDropdown(false); setShowGuestsDropdown(false); }} className="w-full h-full text-left px-4 py-3 bg-white hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0"><CalendarIcon className="w-4 h-4 text-blue-600" /></div><div className="flex-1 min-w-0"><p className="text-xs text-gray-500 uppercase tracking-wide">CHECK-IN</p><p className="font-semibold text-gray-900 truncate">{checkIn ? format(checkIn, "dd MMM''yy") : "Select Date"}</p><p className="text-xs text-gray-500 truncate">{checkIn ? format(checkIn, "EEEE") : ""}</p></div><ChevronDown className="w-4 h-4 text-gray-500 shrink-0" /></div>
                  </button>
                  {showCheckInDropdown && (<div className="absolute top-full left-0 mt-2"><DatePickerDropdown title="Select Check-In Date" onClose={handleCheckInClose} onDateSelect={handleCheckInSelect} nextStep={() => setShowCheckOutDropdown(true)} selectedDate={checkIn} autoOpen={autoOpenCheckIn} /></div>)}
                </div>

                <div className="relative border-b md:border-b-0 md:border-r border-gray-200">
                  <button onClick={() => { setShowLocationDropdown(false); setShowCheckInDropdown(false); setShowCheckOutDropdown(!showCheckOutDropdown); setShowGuestsDropdown(false); }} className="w-full h-full text-left px-4 py-3 bg-white hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0"><CalendarIcon className="w-4 h-4 text-blue-600" /></div><div className="flex-1 min-w-0"><p className="text-xs text-gray-500 uppercase tracking-wide">CHECK-OUT</p><p className="font-semibold text-gray-900 truncate">{checkOut ? format(checkOut, "dd MMM''yy") : "Select Date"}</p><p className="text-xs text-gray-500 truncate">{checkOut ? format(checkOut, "EEEE") : ""}</p></div><ChevronDown className="w-4 h-4 text-gray-500 shrink-0" /></div>
                  </button>
                  {showCheckOutDropdown && (<div className="absolute top-full left-0 mt-2"><DatePickerDropdown title="Select Check-Out Date" onClose={handleCheckOutClose} onDateSelect={handleCheckOutSelect} nextStep={() => setShowGuestsDropdown(true)} selectedDate={checkOut} autoOpen={autoOpenCheckOut} /></div>)}
                </div>

                <div className="relative">
                  <button onClick={() => { setShowLocationDropdown(false); setShowCheckInDropdown(false); setShowCheckOutDropdown(false); setShowGuestsDropdown(!showGuestsDropdown); }} className="w-full h-full text-left px-4 py-3 bg-white hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0"><Users className="w-4 h-4 text-blue-600" /></div><div className="flex-1 min-w-0"><p className="text-xs text-gray-500 uppercase tracking-wide">ROOMS & GUESTS</p><p className="font-semibold text-gray-900 truncate">{guestsDisplay}</p></div><ChevronDown className="w-4 h-4 text-gray-500 shrink-0" /></div>
                  </button>
                  {showGuestsDropdown && (<div className="absolute top-full left-0 mt-2"><GuestsDropdown onClose={handleGuestsClose} onSelectGuests={(guestsInfo) => {}} travellers={travellers} onTravellersChange={setTravellers} autoOpen={autoOpenGuests} /></div>)}
                </div>
              </div>

              <div className="border-t md:border-t-0 md:border-l border-gray-200">
                <button onClick={handleSearch} disabled={!isSearchEnabled || loading} className={`w-full h-full py-4 px-6 bg-orange-600 hover:bg-orange-700 text-white font-bold text-lg rounded-b-2xl md:rounded-r-2xl md:rounded-bl-none transition-all shadow-lg flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed ${isSearchEnabled && !loading && "hover:shadow-xl"}`}>
                  {loading ? (<><Loader className="w-6 h-6 animate-spin" /><span>Loading...</span></>) : (<><Search className="w-6 h-6" /><span className="hidden md:inline">Search Hotels</span><span className="md:hidden">Search</span></>)}
                </button>
              </div>
            </div>
          </div>
        ...
        {showResults && (
          <div id="hotel-results" className="max-w-7xl mx-auto px-4 pb-20">
            {searchData && (<div className="bg-white rounded-xl shadow p-4 mb-6"><div className="flex items-center gap-4 text-sm flex-wrap"><span className="font-semibold">{searchData.location}</span><span>|</span><span>{format(searchData.checkIn, "dd MMM yyyy")}</span><span>-</span><span>{searchData.checkOut ? format(searchData.checkOut, "dd MMM yyyy") : "Flexible"}</span><span>|</span><span>{guestsDisplay}</span><span className="ml-auto text-orange-600 font-semibold">{searchData.totalHotels} Hotels Found</span></div></div>)}
                  ...
                  if (loading) {
                    ...
                    return (
                      <div className="text-center py-12 bg-white rounded-xl shadow-md">
                        ...
                        <button onClick={() => { setLocation("Mumbai"); handleSearch(); }} className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-2 px-6 rounded-lg">
                        ...
                        <div key={index} className="bg-white rounded-lg shadow border p-4 hover:shadow-md transition-shadow">
                          <div className="flex">
                            ...
                            <div className="flex-1">
                              ...
                              <button onClick={() => handleBookNow(hotel)} className="text-blue-600 hover:text-orange-600 text-xs font-medium mt-1">
                                View Details →
                              </button>
                            </div>
                          </div>
