// TourPdfDocumentinternational.tsx - MODIFIED VERSION
...
interface TourData {
    ...
    structured_currency?: Array<{
        local_currency?: string;
        currency_conversion_1?: string;
        currency_conversion_2?: string;
        city_name?: string;
        india_time?: string;
        local_time?: string;
    }>;
...
const Exhibitioninternatonalpdf: React.FC<TourPdfDocumentProps> = ({
    tour,
    tourType,
    isGroupTour,
    selectedCostMonth,
    ...
    // Function to render Currency tab content
    const renderCurrency = () => {
        if (!tour?.structured_currency || tour.structured_currency.length === 0) {
            return (
                <View style={styles.highlight}>
        ...
        return (
            <View>
                <View style={styles.visaTableContainer}>
                    {/* Header */}
                    <View style={styles.visaTableHeader}>
                        <Text style={[styles.visaTableHeaderCell, { width: '20%' }]}>Local Currency</Text>
                        <Text style={[styles.visaTableHeaderCell, { width: '20%' }]}>Conversion 1</Text>
                        <Text style={[styles.visaTableHeaderCell, { width: '20%' }]}>Conversion 2</Text>
                        <Text style={[styles.visaTableHeaderCell, { width: '15%' }]}>City Name</Text>
                        <Text style={[styles.visaTableHeaderCell, { width: '12%' }]}>India Time</Text>
                        <Text style={[styles.visaTableHeaderCell, { width: '13%' }]}>Local Time</Text>
                    </View>
                    ...
                    {tour.structured_currency.map((item, index) => (
                        ...
                        >
                            <Text style={[styles.visaTableCell, { width: '20%', textAlign: 'center' }]}>
                                {item.local_currency || '-'}
                            </Text>
                            <Text style={[styles.visaTableCell, { width: '20%', textAlign: 'center' }]}>
                            ...
                            <Text style={[styles.visaTableCell, { width: '15%', textAlign: 'center' }]}>
                                {item.city_name || '-'}
                            </Text>
                            <Text style={[styles.visaTableCell, { width: '12%', textAlign: 'center' }]}>
                                {item.india_time || '-'}
                            </Text>
                            <Text style={[styles.visaTableCell, { width: '13%', textAlign: 'center' }]}>
                                {item.local_time || '-'}
                            </Text>
                        </View>
                    ))}
...
const renderTourCostTable = () => {
  // International tours use departure-based cost (threeStar/fourStar/fiveStar)
  const departures = tour?.departures?.data;
  const hasDepData = departures && departures.length > 0 &&
    departures.some(d =>
      d.threeStar?.twin !== 'NA' || d.fourStar?.twin !== 'NA' || d.fiveStar?.twin !== 'NA'
    );

  // Fallback to old pax-based table if departure cost data is absent
  const hasTourCostData = tour?.tourCost?.tableData && tour.tourCost.tableData.length > 0;

  if (!hasDepData && !hasTourCostData) {
    return (
      <View style={styles.highlight}>
  ...
  // ── Departure-based cost table (International Exhibition) ──
  if (hasDepData) {
    const rows = [
      { label: 'Per pax on Twin Basis',       key: 'twin'   },
      { label: 'Per pax on Triple Basis',      key: 'triple' },
      { label: 'Per pax Single Occupancy',     key: 'single' },
    ];

    return (
      <View>
        {departures
          .filter(dep => dep.month !== 'No Date Available' && dep.month !== 'Invalid Date')
          .map((dep, depIdx) => (
            <View key={depIdx} style={{ marginBottom: 14 }}>
              {/* Departure heading */}
              <View style={{
                backgroundColor: '#0A1D4A',
                padding: 6,
                ...
                <Text style={{ color: 'white', fontSize: 9, fontWeight: 'bold' }}>
                  {dep.fromDate} – {dep.toDate}  |  Status: {dep.status}
                </Text>
              </View>
              ...
              <View style={styles.tableContainer}>
                ...
                <View style={styles.tableHeader}>
                  <Text style={[styles.tableHeaderCell, { width: '40%', textAlign: 'left', paddingLeft: 6 }]}>
                  ...
                  <Text style={[styles.tableHeaderCell, { width: '20%' }]}>Luxury</Text>
                </View>
                ...
                {rows.map((row, i) => (
                  ...
                  >
                    <Text style={[styles.tableCell, { width: '40%', textAlign: 'left', paddingLeft: 6 }]}>
                      {row.label}
                    </Text>
                    <Text style={[styles.tableCell, { width: '20%' }]}>
                      {dep.threeStar?.[row.key] || 'NA'}
                    </Text>
                    <Text style={[styles.tableCell, { width: '20%', color: '#10b981', fontWeight: 'bold' }]}>
                      {dep.fourStar?.[row.key] || 'NA'}
                    </Text>
                    <Text style={[styles.tableCell, { width: '20%' }]}>
                      {dep.fiveStar?.[row.key] || 'NA'}
                    </Text>
                  </View>
                ))}
              </View>
  ...
  // ── Fallback: pax-based table (Domestic / old format) ──
  return (
    <View style={styles.tableContainer}>
      <View style={styles.tableHeader}>
      ...
      {tour.tourCost.tableData.map((row, index) => (
        ...
        >
          <Text style={[styles.tableCell, { width: '16%' }]}>{row.passenger || 'N/A'}</Text>
          <Text style={[styles.tableCell, { width: '16%' }]}>
            {row.standard && row.standard !== 'NA' ? formatPrice(row.standard) : 'NA'}
          </Text>
          <Text style={[styles.tableCell, { width: '16%', color: '#10b981', fontWeight: 'bold' }]}>
            {row.deluxe && row.deluxe !== 'NA' ? formatPrice(row.deluxe) : 'NA'}
          </Text>
          <Text style={[styles.tableCell, { width: '16%' }]}>
            {row.executive && row.executive !== 'NA' ? formatPrice(row.executive) : 'NA'}
          </Text>
          <Text style={[styles.tableCell, { width: '18%', color: '#2563eb' }]}>
            {row.childWithBed && row.childWithBed !== 'NA' ? formatPrice(row.childWithBed) : 'NA'}
          </Text>
          <Text style={[styles.tableCell, { width: '18%', color: '#7c3aed' }]}>
            {row.childNoBed && row.childNoBed !== 'NA' ? formatPrice(row.childNoBed) : 'NA'}
          </Text>
        </View>
      ))}
    </View>
    ...
    // Function to render all departures table (ALWAYS SHOW DEPARTURE TABLE)
    const renderAllDeparturesTable = () => {
        if (!tour?.departures?.data || tour.departures.data.length === 0) {
            return (
                <View style={styles.highlight}>
                    <Text style={styles.text}>No departure dates available</Text>
                </View>
            );
        }
    ...
    // Function to render airlines table
    ...
    const renderEMITable = () => {
        ...
        return (
            <View style={styles.tableContainer}>
                <View style={styles.tableHeader}>
                    <Text style={[styles.tableHeaderCell, { width: '40%' }]}>Loan Amount</Text>
                    <Text style={[styles.tableHeaderCell, { width: '20%' }]}>Months</Text>
                    <Text style={[styles.tableHeaderCell, { width: '40%' }]}>EMI</Text>
                </View>
    ...
    return (
        <Document>
            ...
            <Page size="A4" style={styles.page}>
                ...
                {tour?.description && (
                    <View style={[styles.overviewBox, { marginBottom: 15 }]}>
                        <Text style={[styles.title, { marginBottom: 4 }]}>Exhibition Overview:</Text>
            ...
            <Page size="A4" style={styles.page}>
                <View style={styles.section}>
                    ...
                    {/* DEPARTURE DATES TABLE - ALWAYS SHOWN */}
                    <View style={styles.section}>
                        <Text style={styles.title}>Departure Dates:</Text>
                        {renderAllDeparturesTable()}
                    </View>

                    {/* TOUR COST TABLE - ALWAYS SHOWN (Individual Tour Cost Table) */}
                    <View style={styles.section}>
                        <Text style={styles.title}>Tour Cost:</Text>
                        {renderTourCostTable()}
                    </View>
...
export default Exhibitioninternatonalpdf;